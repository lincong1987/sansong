...
//#}
     ...
     *   @topic: 1
     */
    addTab: function(id, text, size, position, row, close) {
        if (!this.__skin && dhtmlx.skin)
...
//#}
        ...
        var tab = this._createTab(id, text, (size == "*" ? 10 : size || this._s._tabSize), close);
    ...
    _createTab: function(id, text, size, close) {
        var tab = document.createElement("DIV");
        tab.className = 'dhx_tab_element dhx_tab_element_inactive';
        var thml = "<span>" + text + "</span><div></div><div></div><div></div>";
        if (this._close && !!close)
            thml += "<IMG style='" + this._pxc + ":4px; " + this._py + (this.skin == "dhx_web" ? ":7" : ":4") + "px; position:absolute;z-index:11;' src='" + this.imgUrl + this.skin + "/close.png' >";
        tab.innerHTML = thml;
        ...
        if (this._close && !!close) {
            ...
            tab.ondblclick = function(e){
                var id = this.getAttribute("tab_id");
    ...
    _img: function(tag, y, pos, a, b) {
        ...
        if (this._s._vMode == 1 && this.skin == "dhx_terrace")
            tag.style.width = "240px";
...
//#}
    ...
    cells: function(id) {
        ...
        if (!this._content[id]) {
            ...
            if (this.skin == "dhx_web" && d._setPadding)
                d._setPadding(1, "dhxcont_tabbar_dhx_web"); // for headerless cell use d._setPadding(0, "");
    ...
    enableTab: function(id) {
        ...
        if (this._tabs[id].className)
            this._tabs[id].className = this._tabs[id].className.replace(/dhx_tab_element_disabled/g, "");
        ...
        this.obj.adjustContent = function(parentObj, offsetTop, marginTop, notCalcWidth, offsetBottom) {
            ...
            if (offsetBottom) {
                if (!isNaN(offsetBottom))
                    this.dhxcont.style.height = parseInt(this.dhxcont.style.height) - offsetBottom + "px";
            }
            // main window content
...
function dhx_init_tabbars() {
    ...
    for (var i = 0; i < z.length; i++)
        if (z[i].className.indexOf("dhtmlxTabBar") != -1) {
            ...
            if (n.getAttribute("enableForceHiding"))
                w.enableForceHiding(true);
            ...
            if (clrs)
                w.setSkinColors(clrs.split(",")[0], clrs.split(",")[1]);
