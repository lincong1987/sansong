...
function dhtmlXGridObject(id) {
    ...
    this.setSkin = function(name) {
        this.skin_name = name;
        this.entBox.className = "biz-search-grid gridbox gridbox_" + name;
    ...
    this.setColumnSizes = function(gridWidth) {
        ...
        if (this.ftr)
            this.ftr.style.width = isNaN(summ) ? "auto" : summ + "px";
        this.chngCellWidth();
        return summ;
    };
...
dhtmlXGridObject.prototype = {
    ...
    _unsetRowHover: function(e, c) {
        ...
        if ((opts._lahRw) && (opts._lahRw != c)) {
            for (var i = 0; i < opts._lahRw.childNodes.length; i++)
                opts._lahRw.childNodes[i].className = opts._lahRw.childNodes[i].className.replace(opts._hvrCss, "");
            opts._lahRw = null;
        }
    },
                        ...
                        if (optCol.length) {
                            ...
                            if (this.cellType[i] == "clist") {
                                for (var j = 0; j < optCol.length; j++)
                                    resAr[resAr.length] = optCol[j].firstChild
                                ...
                                for (var j = 0; j < optCol.length; j++)
                                    combo.put(optCol[j].getAttribute("value"),
                                            optCol[j].firstChild
                                            ? optCol[j].firstChild.data
                                            : "");
                            }
    ...
    forEachRowA: function(custom_code) {
        for (var a = 0; a < this.rowsBuffer.length; a++) {
            if (this.rowsBuffer[a])
                custom_code.call(this, this._row(a).idd);
        }
    },
    ...
    enableAutoWidth: function(mode, max_limit, min_limit) {
        ...
        if (arguments.length == 1)
            this.objBox.style.overflowX = mode ? (this.entBox.cellwidthtype == "%" ? "auto" : "hidden") : "auto";
    },
    ...
    _process_xml_row: function(r, xml) {
        ...
        if (this._ud_enabled) {
            ...
            for (var i = udCol.length - 1; i >= 0; i--)
                this.setUserData(r.idd, udCol[i].getAttribute("name"), udCol[i].firstChild
    ...
    load: function(url, call, type) {
        ...
        if (this.opts.pager.method.toUpperCase() === "POST") {
            ...
            var pageParam = {};
            //pageParam[this.opts.pager.pagesizeParmName] = this.opts.pager.pageSize;
            //pageParam[this.opts.pager.pageParmName] = this.opts.pager.page;

            this.xmlLoader.loadXML(url, "POST", this.param($.extend(true, this.opts.pager.param, pageParam)));
        } else {
    ...
    _doPagination: function(data, error, originalData) {
        //	if($.progressBar){
        //		$.progressBar.hide(0);
        //		}
        var _grid = this;
        ...
        if (error && !error.flag) {
            ...
            document._GRID_ERROR_INDEX_ = document._GRID_ERROR_INDEX_ && document._GRID_ERROR_INDEX_++ || 1;

            document._GRID_ERROR_CACHE_[document._GRID_ERROR_INDEX_] = {
                _GRID_ERROR_INDEX_: document._GRID_ERROR_INDEX_,
            ...
            document.__SHOW_GRID_ERROR_BY_INDEX_ = document.__SHOW_GRID_ERROR_BY_INDEX_ || function(index) {
                $(".GRID_ERROR").remove();
                var _ERROR_CACHE_ = document._GRID_ERROR_CACHE_[index];
                var temp = ("<!DOCTYPE html>"
                        + "<html>"
                        + "<head><style>body {font-size: 12px;} b{color:#666;} span{margin-left:5px;font-family:Tahoma,Arial,Helvetica,serif;}</style></head>"
                        + "<body>"
                        + "<dl><b>Request URL:</b><span>" + _ERROR_CACHE_.originalData.filePath + "</span></dl>"
                        + "<dl><b>Request Method:</b><span>" + _ERROR_CACHE_.method + "</span></dl>"
                        + "<dl><b>Status Code:</b><span>" + _ERROR_CACHE_.originalData.xmlDoc.status + " " + _ERROR_CACHE_.originalData.xmlDoc.statusText + "</span></dl>"
                        + "<dl><b>Async:</b><span>" + _ERROR_CACHE_.originalData.async + "</span></dl>"
                        + "<dl><b>ReadyState:</b><span>" + _ERROR_CACHE_.originalData.xmlDoc.readyState + "</span></dl>"
                        + "<dl><b>Request Param:</b><span>" + decodeURIComponent($.param(_ERROR_CACHE_.param)) + "</span></dl>"
                        + "<dl><b>Response:</b><span>" + _ERROR_CACHE_.originalData.xmlDoc.response + "</span></dl>"
                        + "<dl><b>ResponseType:</b><span>" + _ERROR_CACHE_.originalData.xmlDoc.responseType + "</span></dl>"
                        + "<dl><b>Content-Length:</b><span>" + (_ERROR_CACHE_.originalData.xmlDoc.response && _ERROR_CACHE_.originalData.xmlDoc.response.length) + "</span></dl>"
                        + "</body></html>");
                var s = window.open("", "", "height=500px,width=800px,top=" + ((top.screen.availHeight - 500) / 2) + "px,left=" + ((top.screen.availWidth - 800) / 2) + "px,toolbar=no,menubar=no,scrollbars=no,resizable=no,location=no,status=no,alwaysRaised=yes,z-look=yes");
                s.document.write(temp);
                s.document.title = "列表错误加载错误 - 响应详细内容";
            };

            $("<div />", {
                html: "<span class='dhxlayout_progress_error_text'><b>" + status + ".</b> 对不起，您请求的数据暂时不可用。 状态:" + statusText + "; <a style='color:#eee' href='javascript:document.__SHOW_GRID_ERROR_BY_INDEX_(" + document._GRID_ERROR_INDEX_ + ");'>查看详细信息</a></span>",
                "class": "dhxlayout_progress dhxlayout_progress_error",
                id: $(this.hdrBox).parent().attr("id") + "_progress",
                css: {
                    top: $(this.objBox).offset().top,
                    left: $(this.objBox).offset().left,
                    height: $(this.objBox).height() - 1,
        ...
        pager.original_id = pager.id.replace("#", "");

        var pagerStr = pager.pageStatMessage != "" ?
                '<ul class="pager-left">\
		<li><a class="pager-first btn" id="pager-first-' + pager.original_id + '" title="第一页"><button type="button"><i class="icon icon-white icon-step-backward"></i></button></a>\
		 <a class="pager-prev btn" id="pager-prev-' + pager.original_id + '" title="上一页"><button type="button"><i class="icon icon-white icon-chevron-left"></i></button></a>\
		 <a class="pager-next btn" id="pager-next-' + pager.original_id + '" title="下一页"><button type="button"><i class="icon icon-white icon-chevron-right"></i></button></a>\
		 <a class="pager-last btn" id="pager-last-' + pager.original_id + '" title="最后一页"><button type="button"><i class="icon icon-white icon-step-forward"></i></button></a>\
		 <input class="pager-input" id="pager-input-' + pager.original_id + '" value="{currentPage}"/><a class="pager-go btn" id="pager-go-' + pager.original_id + '"><button type="button"><i class="icon icon-white icon-search"></i></button></a></li>\
		</ul><ul class="pager-right"><li><span class="pager-detail" id="pager-detail-' + pager.original_id + '">{above}{total}条数据 {above}{pageCount}页 每页{pageSize}条 现在显示第 {currentPage} 页 {xls}{pdf} {print} {showStatus}</span></li>\
	</ul>' :
                '<ul class="pager-left">\
		<li><a class="pager-first btn" id="pager-first-' + pager.original_id + '" title="第一页"><button type="button"><i class="icon icon-white icon-step-backward"></i></button></a></li>\
		<li><a class="pager-prev btn" id="pager-prev-' + pager.original_id + '" title="上一页"><button type="button"><i class="icon icon-white icon-chevron-left"></i></button></a></li>\
		<li><a class="pager-next btn" id="pager-next-' + pager.original_id + '" title="下一页"><button type="button"><i class="icon icon-white icon-chevron-right"></i></button></a></li>\
		<li><a class="pager-last btn" id="pager-last-' + pager.original_id + '" title="最后一页"><button type="button"><i class="icon icon-white icon-step-forward"></i></button></a></li>\
		<li><input class="pager-input" id="pager-input-' + pager.original_id + '" value="{currentPage}"/><a class="pager-go btn" id="pager-go-' + pager.original_id + '"><button type="button"><i class="icon icon-white icon-search"></i></button></a></li>\
		</ul><ul class="pager-right"><li><span class="pager-detail" id="pager-detail-' + pager.original_id + '">' + pager.pageStatMessage + ' {xls}{pdf} </span></li>\
	</ul>';
        ...
        pagerStr = pagerStr
                .replace(/{above}/g, pager.total >= pager.greater ? "大于" : "共")
                .replace(/{pageSize}/g, "<select class='grid-pager-pageSizeArray' id='grid-pager-pageSizeArray-" + pager.original_id + "'>" + _options + "</select>")
                .replace(/{print}/g, "<span class='grid-print-view' id='grid-print-view-" + pager.original_id + "'>打印</span>")
                .replace(/{xls}/, pager.showToXLS == true ? "<span class='grid-toXls' id='grid-toXls-" + pager.original_id + "'>导出</span>" : "")
                .replace(/{pdf}/, pager.showToPDF == true ? "<span class='grid-toPdf' id='grid-toPdf-" + pager.original_id + "'>PDF</span>" : "");
        ...
        pagerStr = typeof(pager.showStatus) != "undefined" && pager.showStatus == true
                ? pagerStr.replace(/{showStatus}/g, ' <span class="grid-print-time-detail" id="grid-print-time-detail-' + pager.original_id + '" onclick="_GRID_FN_.showReqDetail();">耗时</span> ' + (pager.loadingTime / 1000) + '秒')
                : pagerStr.replace(/{showStatus}/g, "");

        $(pager.id).html("<div class='gridPager-container'>" + pagerStr + "</div>");

        var opts = this;

        $("#grid-print-view-" + pager.original_id).bind("click", function() {
            opts.printView();
        });
        $("#pager-first-" + pager.original_id).bind("click", function() {
            opts._pageBuilder('first');
        });
        $("#pager-prev-" + pager.original_id).bind("click", function() {
            opts._pageBuilder('prev');
        });
        $("#pager-next-" + pager.original_id).bind("click", function() {
            opts._pageBuilder('next');
        });
        $("#pager-last-" + pager.original_id).bind("click", function() {
            opts._pageBuilder('last');
        });
        var pageCount = this.opts.pager.pageCount;

        $("#grid-pager-pageSizeArray-" + pager.original_id).change(function() {
            var inputVal = $("#grid-pager-pageSizeArray-" + pager.original_id).val();
            pager.pageSize = parseInt(inputVal);
            opts._pageBuilder('input', 1);
        });
        $("#pager-go-" + pager.original_id).bind("click", function() {
            var inputVal = $.trim($("#pager-input-" + pager.original_id).val());
            if (isNaN(inputVal) || parseInt(inputVal) <= 0 || (pageCount == 1 && parseInt(inputVal) == 1)) {
                $("#pager-input-" + pager.original_id).val(1);
                return false;
            }
            if (parseInt(inputVal) > pageCount) {
                $("#pager-input-" + pager.original_id).val(pageCount);
            }
            opts._pageBuilder('input', $("#pager-input-" + pager.original_id).val());
        });
        $("#grid-toXls-" + pager.original_id).bind("click", function() {
            //try{
            var xls = pager.xls || {};
        ...
        $("#pager-input-" + pager.original_id).focus(function() {
            $(this).select();
        }).keydown(function(e) {
            e = e || e.event;
            if (e.keyCode == 13)
                $("#pager-go-" + pager.original_id).click();
        });
        if (pager.page == 1) {
            $("#pager-first-" + pager.original_id + ",#pager-prev-" + pager.original_id).addClass("btn-gray").find(".icon").addClass("icon-gray");
        } else {
            $("#pager-first-" + pager.original_id + ",#pager-prev-" + pager.original_id).removeClass("btn-gray");
        }

        if (pager.page == pager.pageCount) {
            $("#pager-next-" + pager.original_id + ",#pager-last-" + pager.original_id).addClass("btn-gray").find(".icon").addClass("icon-gray");
        } else {
            $("#pager-next-" + pager.original_id + ",#pager-last-" + pager.original_id).removeClass("btn-gray");
        }

        if (pager.total == 0) {
            $("#pager-next-" + pager.original_id + ", #pager-last-" + pager.original_id + ", #pager-next-" + pager.original_id + ", #pager-first-" + pager.original_id).removeClass("btn-gray").addClass("btn-gray").find(".icon").addClass("icon-gray");
        }
        //autoHeight
        ...
        if (this.opts.pagerBuilder === false) {
            $(pager.id).css({display: "none"});
        }
    ...
    _pageBuilder: function(pageType, inputVal) {
        ...
        if (flag == true) {
            //$(p.id).html("<div class='gridPager-container'><ul><li><span class='pager-loading'>" + p.loadingMassage + "</span></li></ul></div>");
            ...
            var qry = p.method.toUpperCase() === "POST"
                    ? (p.url + "?" + p.pageParmName + "=" + p.page + "&" + p.pagesizeParmName + "=" + p.pageSize)
                    : (p.url + "?" + p.pageParmName + "=" + p.page + "&" + p.pagesizeParmName + "=" + p.pageSize + "&" + param);

//            qry = "";
//            //var param = $.param(p.param);
//            if (p.method.toUpperCase() === "POST") {
//                qry = p.url + (/\/$/.test(p.url) ? "" : "/") + p.page + "/" + p.pageSize + ".json";
//                // 如果url 含有{xxx}时进行渲染
//                if (/\{.*\}/.test(p.url)) {
//                    qry = p.url.replace("{" + p.pageParmName + "}", p.page).replace("{" + p.pagesizeParmName + "}", p.pageSize) + ".json";
//                }
//            } else {
//                qry = p.url + (/\/$/.test(p.url) ? "" : "/") + p.page + "/" + p.pageSize + ".json?" + param;
//                // 如果url 含有{xxx}时进行渲染
//                if (/\{.*\}/.test(p.url)) {
//                    qry = p.url.replace("{" + p.pageParmName + "}", p.page).replace("{" + p.pagesizeParmName + "}", p.pageSize) + ".json?" + param;
//                }
//            }
            ...
            this.clearAndLoad(qry, function(grid) {
                ...
                _progress.fadeOut((typeof(p.fadeOut) == 'undefined' ? 500 : parseInt(p.fadeOut)), function() {
                    _progress.remove();
                    setTimeout(function() {
                        $(grid.objBox).height($(grid.objBox).height() - 1);
                    }, 1);
                });
            }, "json");
        }
    ...
    _process_json: function(data) {
        ...
        //数据过滤
        data = this.opts.pager.dataFilter(data, original);
        ...
        if (!_f) {
            ...
            this._doPagination(data, _error, original);
            //}
        ...
        for (var i = min; i < max; i++) {
            ...
            if (this._ads_count && i - min == this._ads_count) {
                ...
                return this._contextCallTimer = window.setTimeout(function() {
                    ...
                    opts._dataset(i, max);
                    if (!opts._contextCallTimer) {
                        if (opts._context_parsing)
                            opts.callEvent("onXLE", [])
                        else
                            opts._fixAlterCss();
    ...
    dp._clearUpdateFlag = function(rowId) {
        ...
        if (row)
            for (var j = 0; j < this.obj._cCount; j++)
                this.obj.cells(rowId, j).cell.wasChanged = false;
    };
...
(function($) {
    $.fn.grid = function(options) {
        var defaults = {
            ...
            onErrColorboxClose: function() {
            }, //当AJAX异常colorbox点击关闭时的回调
            ...
            pager: {//分页参数定义
                ...
                pageCount: 0, //总页数
                url: "", //ajax请求地址
                root: "rows", //返回的json中用于处理的对象名称
                record: "total", //json中总条数的名称
                pageParmName: 'page', //ajax请求需要发送的当前页key名称
                pagesizeParmName: 'size', //ajax请求需要发送的每页数key名称
                resetPager: function() {
                }, //在doSearch时 建议重置分页
                setParam: function() {
                }, //设置附加查询参数
            ...
            onRowSelect: function(rowId, cellIndex) {
                ...
                $.each(opts.columns, function(i, n) {
                    ...
                    $("#" + key).val(opts.render[key] && (opts.columnType[i] != "date" && opts.columnType[i] != "select") ? opts.render[key](row[key]) : row[key]);
                });
                //隐藏删除按钮
                ...
                $(opts.gridPanel).find(opts.btn.saveBtn).unbind("click").click(function() {
                    ...
                    if (!object) {
                        ...
                        $.each(opts.columns, function(i) {
                            var key = opts.columns[i];
                            var value = opts.render[key] && opts.columnType[i] != "date" ? opts.render[key](object[key]) : object[key];
                            array.push(value);
                        });
            ...
            doFormCheck: function() {
                ...
                function ruleMessageFormat(str, array) {
                    if (!array)
                        return str;
                    if (typeof(array) !== "object") {
                        str = str.replace(/\{0\}/g, array);
                        return str;
                    }
                ...
                $.each(opts.columns, function(i) {
                    ...
                    if (key != "_rownum") {
                        ...
                        if (opts.columnValidate === true) {
                            ...
                            // }
                            //
                            if (!$.isEmptyObject(opts.customerRules || {}) && key in opts.customerRules) {
                                //$.each(opts.customerRules, function(_name, _rule){
                                //opts.customerRules
                                var _rule = $.extend({
                                    meassage: "",
                                    rule: new RegExp(),
                                ...
                                }, opts.customerRules[key] || {});

                                if (!_rule.rule.test(_v)) {
                                    opts.columnValidateFlag = false;
                                    $input.addClass(onErrorClass).focus();
                                    _rule.error(key, columnName, $input);
                                    alert(ruleMessageFormat(_rule.message, _v));
                                    return false;
                                } else {
                                    _rule.success(columnName[i], columnName, $input);
                                }
                                //});
                            }
                            ...
                            //默认长度验证
                            if (typeof opts.columnValidator[i].maxlength != 'undefined' && !isNaN(opts.columnValidator[i].maxlength)) {
                            ...
                            //默认长度验证
                            if (typeof opts.columnValidator[i].minlength != 'undefined' && !isNaN(opts.columnValidator[i].minlength)) {
                            ...
                            //默认min验证
                            if (typeof opts.columnValidator[i]["min"] != 'undefined' && !isNaN(opts.columnValidator[i]["min"])) {
                            ...
                            //默认max验证
                            if (typeof opts.columnValidator[i]["max"] != 'undefined' && !isNaN(opts.columnValidator[i]["max"])) {
                            ...
                            //默认日期验证
                            if (opts.columnValidator[i].date) {
                                if ($.trim(_v) != "" && !/^(\d{1,4})(-|\/)(\d{1,2})\2(\d{1,2})/.test(_v)) {
                                    alert(columnName[i] + " 只能填写日期!");
                                    opts.columnValidateFlag = false;
                                    $input.addClass(onErrorClass).focus();
                                    return false;
                                }
                            ...
                            if (opts.columnValidator[i].tel) {
                                if ($.trim(_v) != "" && !/(\d{3}-\d{8}|\d{4}-\d{7}|\d{4}-\d{8}|\d{7}|\d{8}|\d{10})/.test(_v)) {
                            ...
                            if (opts.columnValidator[i].accept) {
                                ...
                                if ($.inArray(_v, _accept) == -1) {
                                    alert(columnName[i] + " 只能填写 " + _accept.join("|").replace(/\|/g, ",") + " 其中的值!");
                                    opts.columnValidateFlag = false;
                                    $input.addClass(onErrorClass).focus();
                                    return false;
                                }
                            ...
                            //默认acceptLength验证
                            if (opts.columnValidator[i].acceptLength) {
                                var _acceptLength = opts.columnValidator[i].acceptLength;
                                _acceptLength = (typeof _acceptLength == "string" ? _acceptLength.replace(/,/g, '|') : _acceptLength.join("|")).split("|");
                                if ($.inArray(_v.length.toString(), _acceptLength) == -1) {
                                    alert(columnName[i] + " 只能填写长度为 " + _accept.join("|").replace(/\|/g, ",") + " 的值!");
                                    opts.columnValidateFlag = false;
                                    $input.addClass(onErrorClass).focus();
                                    return false;
                                }
                            ...
                            if (opts.columnValidator[i].email) {
                                var value = _v;
                                if ($.trim(_v) != "" && !/^((([a-z]|\d|[!#\$%&'\*\+\-\/=\?\^_`{\|}~]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])+(\.([a-z]|\d|[!#\$%&'\*\+\-\/=\?\^_`{\|}~]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])+)*)|((\x22)((((\x20|\x09)*(\x0d\x0a))?(\x20|\x09)+)?(([\x01-\x08\x0b\x0c\x0e-\x1f\x7f]|\x21|[\x23-\x5b]|[\x5d-\x7e]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(\\([\x01-\x09\x0b\x0c\x0d-\x7f]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF]))))*(((\x20|\x09)*(\x0d\x0a))?(\x20|\x09)+)?(\x22)))@((([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])*([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])))\.)+(([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])*([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])))\.?$/i.test(value)) {
                            ...
                            if (opts.columnValidator[i].url) {
                                var value = _v;
                                if ($.trim(_v) != "" && !/^(https?|ftp):\/\/(((([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(%[\da-f]{2})|[!\$&'\(\)\*\+,;=]|:)*@)?(((\d|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])\.(\d|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])\.(\d|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])\.(\d|[1-9]\d|1\d\d|2[0-4]\d|25[0-5]))|((([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])*([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])))\.)+(([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])*([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])))\.?)(:\d*)?)(\/((([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(%[\da-f]{2})|[!\$&'\(\)\*\+,;=]|:|@)+(\/(([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(%[\da-f]{2})|[!\$&'\(\)\*\+,;=]|:|@)*)*)?)?(\?((([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(%[\da-f]{2})|[!\$&'\(\)\*\+,;=]|:|@)|[\uE000-\uF8FF]|\/|\?)*)?(\#((([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(%[\da-f]{2})|[!\$&'\(\)\*\+,;=]|:|@)|\/|\?)*)?$/i.test(value)) {
            ...
            dataManage: {//数据管理器
                ...
                // 获取行对象
                getRow: function(value, key) {
                    var param = key || "id";
                    var me = this;
                    return $.grep(me.list, function(n, i) {
                        return value == n[param];
                    });
                }
            },
            doLoad: function() {
                ...
                var grid = opts.grid;
                // avaialble: str, int, date, na or function object for custom sorting
                if (!opts.setColSorting || opts.setColSorting.replace(/\s+/g, "") === "") {
                    opts.setColSorting = [];
                    $.each(opts.columns, function(i, n) {
                ...
                grid.setColSorting(opts.setColSorting.replace(/\s+/g, ""));
            ...
            getJson: function() {
                ...
                //$(pager.id).html("<div class='gridPager-container'><ul><li><span class='pager-loading'>" + pager.loadingMassage + "</span></li></ul></div>");
                var _progress = $("#" + id + "_progress").length == 1 ? $("#" + id + "_progress") : $("<div class='dhxlayout_progress' id='" + id + "_progress'></div>");
                _progress.css({
                ...
                var qry = pager.method.toUpperCase() === "POST"
                        ? pager.url + "?" + pager.pageParmName + "=" + pager.page + "&" + pager.pagesizeParmName + "=" + pager.pageSize
                        : pager.url + "?" + pager.pageParmName + "=" + pager.page + "&" + pager.pagesizeParmName + "=" + pager.pageSize + "&" + param;

//                var qry = "";
//                var param = $.param(pager.param);
//                if (pager.method.toUpperCase() === "POST") {
//                    qry = pager.url + (/\/$/.test(pager.url) ? "" : "/") + pager.page + "/" + pager.pageSize + ".json";
//                    // 如果url 含有{xxx}时进行渲染
//                    if (/\{.*\}/.test(pager.url)) {
//                        qry = pager.url.replace("{" + pager.pageParmName + "}", pager.page).replace("{" + pager.pagesizeParmName + "}", pager.pageSize) + ".json";
//                    }
//
//                } else {
//                    qry = pager.url + (/\/$/.test(pager.url) ? "" : "/") + pager.page + "/" + pager.pageSize + ".json?" + param;
//
//                    if (/\{.*\}/.test(pager.url)) {
//                        qry = pager.url.replace("{" + pager.pageParmName + "}", pager.page).replace("{" + pager.pagesizeParmName + "}", pager.pageSize) + ".json?" + param;
//                    }
//                }
        ...
        if ($(opts.pagerId).length == 0) {
            $gridPager = $('<div id="' + opts.self.attr("id") + 'Pager" class="gridPager"></div>');
        ...
        var pager = {//分页参数定义
            ...
            xlsURL: "",
            xls: {
                url: "/platform-webapp-excel/generate",
                name: "Excel导出 - " + (new Date().getFullYear()) + "-" + (new Date().getMonth() + 1) + "-" + (new Date().getDate()),
                mode: "",
                footer: "",
                rows: ""
            },
...

{
    ...
    if (this.editor) {
        if (src.tagName && (src.tagName == "INPUT" || src.tagName == "TEXTAREA"))
            return;
        this.editStop();
    }
    ;
...

{
    ...
    var X = event.clientX - pos[0] + document.body.scrollLeft;
    var Y = event.clientY - pos[1] + document.body.scrollTop;
    if ((Math.abs(this._init_pos[0] - X) < 5) && (Math.abs(this._init_pos[1] - Y) < 5))
        return this._HideSelection();
...
dhtmlXGridObject.prototype._serializeRowToCVS = function(r, i, start, end) {
    ...
    if (!r) {
        r = this.render_row(i)
        if (this._fake && !this._fake.rowsAr[r.idd])
            this._fake.render_row(i);
    }
    ;
...
dhtmlXGridObject.prototype.getXLS = function(path) {
    if (!this.xslform) {
        ...
        this.xslform.action = (path || "") + "xlsAction.do";
...
dhtmlXGridObject.prototype.printView = function(before, after) {
    ...
    for (var i = 0; i < row_length; i++) {
        ...
        if (this.rowsCol[i] && this.rowsCol[i].style.display == "none")
            continue;
        var row_id
        if (this.rowsCol[i])
        ...
        for (var j = 0; j < col_length; j++) {
            ...
            if (this.rowsAr[row_id] && this.rowsAr[row_id].tagName == "TR") {
                ...
                if (c._setState)
                    ...
                    value = c.getContent();
                else if (c.getImage || c.combo)
                    var value = c.cell.innerHTML;
                else
                    var value = c.getValue() || c.cell.innerHTML || 0;
            } else
