 ...
 *
 * editor lc
 * $Id: firebird-1.0.1.js 1269 2013-08-12 06:56:49Z lincong $
 *
...
(function() {
    ...
    function _hashChangeCallBack() {
        ...
        console.log("@private _hashChangeCallBack>>", _curHash);
    }
    ...
    function _checkHash() {
        ...
        console.log("@private _checkHash", hash, _curHash);
        if (hash !== _curHash) {
    ...
    function listen(callBack) {
        ...
        console.log("@private listen", _curHash);
        ...
        if (firebird.browser.ie) {
            ...
            setInterval(_checkHash, 100);
            console.log("@private listen >> firefox");
        } else {
            ...
            console.log("firebird.history.listen>>>", _curHash, location.hash.replace(/^#/, ''));
        }
    }
 ...
 * console.log("key1 is d%, key2 is d%", firebird.data.set("key1"), firebird.data.set("key2"));
 * step2:
 * firebird.data.clear();
 * console.log(console.log("key1 is d%, key2 is d%", firebird.data.set("key1"), firebird.data.set("key2")););
 */
...
firebird.api.message = function(message) {
    ...
    if (firebird.api.proxy === "") {
        console.log("firebird.api.proxy is undefined");
        return;
    }
...
firebird.api.send = function(type, options) {
    ...
    if (firebird.ie && src.length > 2000) {
        console.log("firebird.api.proxy: Message Body Is Too Large.");
        return;
    }
    ...
    console.log("proxy>>> ", firebird.api.proxy + "#" + hash);
};
...
firebird.form.post = function(_iid) {
    ...
    var form = $("<form method='post' id='seq_form_" + seq + "' name='seq_form_" + seq + "' target='seq_iframe_" + seq + "' style='display: none'></form>").append(poster.data).appendTo("body");
