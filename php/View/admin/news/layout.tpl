...
<html>
    <head>
        ...
        <script type="text/javascript">
            $(function() {
                //icinfo.namespace("grid");
                //icinfo.grid = $.fn.grid;

                $("#viewGrid").grid({
                    setImagePath: "${$smarty.const.WEB_HOST}js/dhtmlx/grid/imgs/",
                    url: '${$smarty.const.WEB_HOST}php/Action/admin/news/newsAction.php',
                    root: "rows",
                    setInitWidths: "40,40,300,100,120,120",
                    setColAlign: "left, center, left, center,center, center,left,left, center",
                    setHeader: "<ul style='text-align:left; padding-left:3px'><input type='checkbox' onclick='doSwitchAllChecked();' id='delAll'><ul>,序号,标题,是否显示,新闻类型,发布时间",
                    columns: ["id", "_rownum", "title", "display", "type", "time"],
                    render: {
                        id: function(val, row) {
                            return "<input value='" + val + "' class='chb gridChk' type='checkbox' />";
                        },
                        type: function(val) {
                            return val == "company" ? "新闻动态" : "行业资讯";
                        },
                        title: function(val, row) {
                            return "<a href='${$smarty.const.WEB_HOST}php/Action/client/news/newsAction.php?ac=" + row.type + "_news&id=" + row.id + "' target='_blank'>" + val + "<a>";
                        },
                ...
                icinfo.namespace("timer.setSizes");
                icinfo.timer.setSizes = null;
                $(window).resize(function() {
                    clearTimeout(icinfo.timer.setSizes);
                    icinfo.timer.setSizes = setTimeout(function() {
                        $("#viewGrid").data("grid").grid.setSizes();
                    }, 1000);
                });
                try {
            ...
            function doAddRow() {
                ...
                var url = "${$smarty.const.WEB_HOST}php/Action/admin/news/newsAction.php?" + $.param({
                    ac: "edit",
                    _id: _id
                });
            ...
            <div class="listBox2_new thBg">
                <form id="searchForm" onsubmit="return false;">
                    <div class="searchbg">
                        <table>
                            <tr>
                                <td width="80" class="texR">标题：</td>
                                <td width="180">
                                    <input id="title" type="text" class="ie6Input" style="width:98%;"/>
                                </td>
                                <td width="80" class="texR">新闻类型：</td>
                                <td>
                                    <select id="type">
